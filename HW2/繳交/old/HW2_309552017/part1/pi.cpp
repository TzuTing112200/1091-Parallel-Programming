    ...
    */
    ...
    for(long long int toss = 0; toss < part_number_of_tosses; toss += 8)
    {
        ...
        */
        ...
        rand_num = rand_r(&seed);
        _nums[0] = (float)(rand_num & 0xffff) / 0xffff;
        
        rand_num = rand_r(&seed);
        _nums[1] = (float)(rand_num & 0xffff) / 0xffff;
        
        rand_num = rand_r(&seed);
        _nums[2] = (float)(rand_num & 0xffff) / 0xffff;
        
        rand_num = rand_r(&seed);
        _nums[3] = (float)(rand_num & 0xffff) / 0xffff;
        
        rand_num = rand_r(&seed);
        _nums[4] = (float)(rand_num & 0xffff) / 0xffff;
        
        rand_num = rand_r(&seed);
        _nums[5] = (float)(rand_num & 0xffff) / 0xffff;
        
        rand_num = rand_r(&seed);
        _nums[6] = (float)(rand_num & 0xffff) / 0xffff;
        
        rand_num = rand_r(&seed);
        _nums[7] = (float)(rand_num & 0xffff) / 0xffff;
        
        _nums = _mm256_mul_ps(_nums, _nums);
        ...
        if((_nums[0] + _nums[1]) < 1.0) temp_number_in_circle++;
        if((_nums[2] + _nums[3]) < 1.0) temp_number_in_circle++;
        if((_nums[4] + _nums[5]) < 1.0) temp_number_in_circle++;
        if((_nums[6] + _nums[7]) < 1.0) temp_number_in_circle++;
        
        if((_nums[0] + _nums[4]) < 1.0) temp_number_in_circle++;
        if((_nums[1] + _nums[5]) < 1.0) temp_number_in_circle++;
        if((_nums[2] + _nums[6]) < 1.0) temp_number_in_circle++;
        if((_nums[3] + _nums[7]) < 1.0) temp_number_in_circle++;
...
float getPI()
{
    ...
    if(number_of_CPU_cores > 4 || number_of_CPU_cores < 3)  part_number_of_tosses *= 1.75;
    part_number_of_tosses = part_number_of_tosses / 8 * 8;
